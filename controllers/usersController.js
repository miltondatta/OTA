...
exports.updateUsersBalance = async (req, res) => {
    try {
        const {user_id, user_name, balance} = req.body.update_info;
        const status = await user.update({balance : Sequelize.literal('balance + '+ balance)}, {where : {id : user_id}});
        if (!status) return res.status(400).json({msg : 'Please try again!'});
        
        return res.status(200).json({msg : 'Amount added successfully!'});
    } catch (err) {
        console.error(err.message);
        return res.status(500).json({msg : 'Server Error!'});
    }
